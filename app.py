...
from mas_engine_v4 import MultiAgentSystemEngineSBCE
...
(tab_dashboard, tab_mapping, tab_historical, tab_siting, tab_mas, 
    ...
    with col_sit_map:
        ...
        with st.expander("🛠️ Ajuste Fino de Calibração do Mapa de Santa Catarina"):
            ...
            with col_adj3:
                ...
                map_source_type = st.radio("Selecione o Mapa Base de SC", ["Vetorizado (Wikimedia)", "Mapa Anexado (Local - WebP)"], index=0)
...
with tab_mas:
    ...
    st.dataframe(
        df_plants.style.format({
            ...
            "Payback (Anos)": "{:,.2f}" if isinstance(df_plants["Payback (Anos)"].iloc[0], (int, float)) else "{}"
        }),
...
with tab_financing:
    ...
    **Plataforma de Linhas de Crédito Sustentável no Brasil** da *Climate Policy Initiative (CPI)*. 
    ...
    with col_f1:
        ...
        capex_financiado = st.slider("Valor do Investimento a Financiar (R$)", min_value=500000, max_value=25000000, value=3000000, step=500000)
        prazo_comercial = st.number_input("Taxa de Juros de Linhas Comerciais Tradicionais (% a.a.)", value=15.0, step=0.5)
